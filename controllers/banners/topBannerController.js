...
const addTopBanner = async (req, res) => {
    ...
    try {
        const baners = await TopBanner.find({ sortOrder: { $gt: sortOrder } });
        if (baners.length) {
            await Promise.all(
                baners.map(async (banner) => {
                    banner.sortOrder = +1;
                    await banner.save();
                })
            );
...
const items = req.body
        try{
            await Promise.all(items.map(async item => {
                ...
                if(item.expirationDate) foundItem.expirationDate = item.expirationDate
...
const getTopBanners = async ( req ,res ) => {
    ...
    try{
        const topBanners = await TopBanner.find().skip(Number(offset)).limit(Number(limit)).sort({sortOrder:-1})
        return res.status(200).json({banners:topBanners , total:topBanners.length})
    }catch(e){
        return res.status(500).json({message:e.message})
    }
}
module.exports = { addTopBanner , editTopBanners , deleteTopBanners , getTopBanners};
